...
flabel(5.66, 4.75, "5-ALA;\nsuccinate up", fs=6.8)

# ---- routed connectors (every line checked against box boundaries) ----
# free channels: top strip y>7.07 (use 7.25); r1-r2 strip 5.87-6.15 (use 6.0);
# r3-r4 strip 3.35-3.6 (use 3.47); bottom strip <1.0 (use 0.7);
# left margin x=-0.5; right margin x=7.75 (c4 ends 7.37); c3-c4 gap 5.57-5.75 (use 5.66)
# 1) published genus stats -> M2b (top strip -> left margin)
arrow(c4c, r1+h1, c4c, 7.25)
arrow(c4c, 7.25, -0.5, 7.25)
arrow(-0.5, 7.25, -0.5, r4+h4/2)
arrow(-0.5, r4+h4/2, c1, r4+h4/2)
flabel(-0.5, 5.0, "genus differential\nstatus (P values)", rot=90, fs=7.5)

# 2) M2b -> M2c (bottom strip -> right margin -> r1-r2 strip)
arrow(c1c, r4, c1c, 0.7)
arrow(c1c, 0.7, 7.75, 0.7)
arrow(7.75, 0.7, 7.75, 6.0)
arrow(7.75, 6.0, c3+w, 6.0)
arrow(c3+w, 6.0, c3+w, r2+h2)
flabel(7.75, 3.6, "metabolite list\n(49 links)", rot=90, fs=7.5)

# 3) M2c -> M4b (down the c3-c4 gap -> r3-r4 strip -> into M4b top)
arrow(c3+w, r2+0.2, 5.66, 3.47)
arrow(5.66, 3.47, c2c, 3.47)
arrow(c2c, 3.47, c2c, r4+h4)
flabel(4.5, 3.47, "sensor gene list (413)", fs=7)
